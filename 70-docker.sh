...
run "add user to docker group" $SUDO usermod -aG docker "$USERNAME"
...
if contains "$WANT" "comfyui"; then
	...
	$SUDO mkdir -p /home/ai/comfyui/models /home/ai/comfyui/output
	...
	if soft "pull comfyui image" $SUDO docker compose -f "$CU/compose.yaml" pull; then
		soft "start comfyui" $SUDO docker compose -f "$CU/compose.yaml" up -d
		note "comfyui at http://127.0.0.1:8188"
	else
...
if contains "$WANT" "portainer"; then
	...
	note "portainer at https://127.0.0.1:9443"
