...
if ip link show virbr0 &>/dev/null; then
	NIC=$(ip route show default | awk '{print $5}' | sed -n 1p)
...
sudo iptables -L FORWARD -n | sed -n 1p
...
check "forward drop"  sh -c 'sudo iptables -L FORWARD -n | sed -n 1p | grep -q DROP'


check "libvirtd active"   systemctl is-active --quiet libvirtd

check "default net up"    sh -c 'sudo virsh net-info default'
